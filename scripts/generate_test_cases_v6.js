...
/**
 * BLU Test Case Generator v6.0
 *
 * Key changes from v5:
 * - Primary mapping: utterance -> KB question (keyword overlap) NOT bot reply -> KB answer
 * - Validation: bot reply in same session used as confidence booster/reducer
 *   - Substantive bot reply with L2-specific keywords -> confidence +20
 *   - Fallback/error bot reply -> skip session entirely
 * - Insurance repo excluded (chatbot-flag=yes = 0, all in JSONs anyway)
 * - Loan repo: only 42 unique entries not in JSONs are added
 * - Expanded cross-module reject rules
 * - Old CSV: gap-fill only for L2s with zero new dump coverage
 * - KB verbatim: gap-fill for L3s with zero real user coverage
 *
 * Output: data/blu_test_cases_v6.json
 */
...
const OUTPUT_JSON = path.resolve('data/blu_test_cases_v6.json');
...
// L2-specific keyword validators — used to boost confidence when bot reply contains these
const L2_VALIDATORS = {
  'emi network card':        ['emi card','insta emi','network card','card limit','card block','card unblock','card number','cvv'],
  'health emi network card': ['health card','health emi','medical'],
  'personal flexi loan':     ['flexi','drawdown','withdrawal','flexi loan'],
  'business secured loan':   ['business secured','bsl','mortgage'],
  'consumer loan':           ['consumer loan','product loan','emi loan'],
  'two wheeler loan':        ['two wheeler','bike loan','scooter'],
  'gold loan':               ['gold loan','gold ornament'],
  'home loan':               ['home loan','housing loan','bhfl','mortgage'],
  'fixed deposit':           ['fixed deposit','fd','maturity','cumulative'],
  'sdp':                     ['sdp','systematic deposit'],
  'insurance services':      ['insurance','policy','premium','claim'],
  'genericbbpsqueries':      ['bill pay','bbps','electricity','recharge'],
  'genericupiqueries':       ['upi','bhim','vpa','p2p','p2m'],
  'genericwalletsqueries':   ['wallet','cashback','topup'],
  'fastag':                  ['fastag','toll','highway'],
  'profile services':        ['profile','mobile number','address','pan','name change'],
};
...
const CROSS_MODULE_REJECTS = [
  ...
  { queryPattern: /change my mobile|update mobile|mobile update|number update/i,
    rejectL2: /emi network card|health emi|business secured|consumer loan/i },
  { queryPattern: /salary|staff|employee|payroll/i,
    rejectL2: /emi network card|health emi|personal flexi/i },
];
...
const FALLBACK_PATTERNS = /temporary issue|try again|facing a temporary|no active relation|great news.*explore|what do you want to do today|please enter your mobile|please provide your 6 digit|please select the relation|hi user|hi, i'm blu|we are unable to process/i;

function isSubstantiveReply(text) {
  if (!text || text.trim().length < 20) return false;
  return !FALLBACK_PATTERNS.test(text);
}
...
// Validate bot reply against matched L2 — returns confidence boost
function validateBotReply(botReply, matchedL2) {
  if (!isSubstantiveReply(botReply)) return -50; // penalise fallback
  const l2lower = matchedL2.toLowerCase();
  const replyLower = botReply.toLowerCase();
  const validators = L2_VALIDATORS[l2lower] || [];
  const matches = validators.filter(kw => replyLower.includes(kw)).length;
  if (matches > 0) return 20; // boost if L2-specific keywords found in reply
  return 0;
}
...
// Loan repo: only add questions NOT already in JSONs
...
console.log('  Loan repo unique additions: ' + (groundTruth.length - beforeLoan));
console.log('  Total KB: ' + groundTruth.length);

// ── STEP 2: Parse new chat dump — utterance→KB question matching ──────────────

console.log('\n📱 Step 2: New chat dump (utterance-to-KB matching)...');
...
const newDumpMapped = [];
let skippedFallback = 0, skippedNoise = 0;
...
newDumpRows.forEach(row => {
  ...
  const turns = entries.map(entry => {
    ...
    return { customerMsg, botReply };
  }).filter(t => t.customerMsg && t.botReply);

  // Skip sessions where ALL bot replies are fallback/noise
  const substantiveTurns = turns.filter(t => isSubstantiveReply(t.botReply));
  if (substantiveTurns.length === 0) { skippedFallback++; return; }

  // Process each turn in this session
  turns.forEach(({ customerMsg, botReply }) => {
    if (NOISE.test(customerMsg)) { skippedNoise++; return; }
    if (customerMsg.length < 4) return;

    const queryTokens = tokenize(customerMsg);
    if (!queryTokens.length) return;

    // PRIMARY: match utterance -> KB question
    const inferredL1  = inferL1(customerMsg);
    const candidates  = inferredL1 ? groundTruth.filter(e => e.l1 === inferredL1) : groundTruth;
    if (!candidates.length) return;

    let bestEntry = null, bestScore = 0;
    for (const e of candidates) {
      const score = keywordOverlap(queryTokens, e.question_tokens);
      if (score > bestScore) { bestScore = score; bestEntry = e; }
    }
    if (!bestEntry || bestScore < 5) return;
    if (isCrossModuleReject(customerMsg, bestEntry.l2)) return;

    // VALIDATE: use bot reply to boost/penalise confidence
    const boost = validateBotReply(botReply, bestEntry.l2);
    const finalConfidence = Math.min(100, Math.max(0, bestScore + boost));

    // Skip if bot gave fallback for this specific turn AND confidence is low
    if (!isSubstantiveReply(botReply) && finalConfidence < LOW_CONF_THRESHOLD) return;

    const mappingType = finalConfidence >= LOW_CONF_THRESHOLD
      ? 'new_dump'
      : 'new_dump_low_confidence';
...
console.log('  Sessions processed: ' + newDumpRows.length);
console.log('  Sessions skipped (all-fallback): ' + skippedFallback);
console.log('  Utterances mapped: ' + newDumpMapped.length);

// ── STEP 3: Old CSV — gap-fill for uncovered L2s ──────────────────────────────

console.log('\n📊 Step 3: Old CSV gap-fill...');

const coveredL2s = new Set(newDumpMapped.map(m => m.matched.l1 + '::' + m.matched.l2));
...
uniqueOldQueries.forEach(query => {
  ...
  if (coveredL2s.has(best.l1 + '::' + best.l2)) return;
  oldCsvMapped.push({
    utterance: query, matched: best, confidence: bestS,
    mapping_type: bestS >= LOW_CONF_THRESHOLD ? 'old_csv' : 'old_csv_low_confidence',
    source: 'old_csv',
  });
  oldP++;
  if (oldP % 10000 === 0) console.log('    ' + oldP.toLocaleString() + '...');
});
...
selectedRealUser.forEach(mq => {
  ...
  testCases.push({
    ...
    module: e.module, l1: e.l1, l2: e.l2, l3: e.l3,
    utterance: mq.utterance,
    expected_answer: e.answer, expected_key_phrases: e.key_phrases,
    cta_expected: e.cta_expected, cta_label: e.cta_label,
    scoring_type: e.scoring_type, is_multi_turn: e.is_multi_turn,
    mapping_type: mq.mapping_type, mapping_confidence: mq.confidence,
    source: mq.source, chat_id: mq.chat_id || null,
  });
});
...
verbatimCases.forEach(e => {
  testCases.push({
    ...
    cta_expected: e.cta_expected, cta_label: e.cta_label,
    scoring_type: e.scoring_type, is_multi_turn: e.is_multi_turn,
    mapping_type: 'kb_verbatim', mapping_confidence: 100,
    source: 'knowledge_base', chat_id: null,
  });
});
...
console.log('\n' + '='.repeat(60));
console.log('BLU Test Cases v6 - Complete');
